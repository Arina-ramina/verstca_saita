...
def create_table():
    ...
    conn = psycopg2.connect(host="localhost", database="postgres",
                            user="postgres", password="2310")
    conn.autocommit = True
    cur = conn.cursor()

    cur.execute("DROP DATABASE IF EXISTS SQL_HH")
    cur.execute("CREATE DATABASE SQL_HH")

    conn.close()

    conn = psycopg2.connect(host="localhost", database="SQL_HH",
                            user="postgres", password="2310")
...
def add_to_table(employers_list):
    ...
    with psycopg2.connect(host="localhost", database="SQL_HH",
                          user="postgres", password="2310") as conn:
        with conn.cursor() as cur:
